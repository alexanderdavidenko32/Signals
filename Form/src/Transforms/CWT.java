...
//            System.out.println("CWT at " + b + ": " + cwt[b].toString());
    ...
    public Complex mhat(double t) {

        double result = (1. - t * t) * Math.exp( -((t * t) / 2.) );
        Complex e = new Complex(0, 0);
        Complex third = e.exp();

        return new Complex(result * third.real(), result * third.imag());
    }

    public Complex gaus3p(double t) {

        double result = ( Math.pow(t, 3) - 3 * t ) * Math.exp( -((t * t) / 2.) );
        Complex e = new Complex(0, 2 * Math.PI * fc * t);
        Complex third = e.exp();
